function AudioSynthView() {
	...
	var fnPlayNote = function(note, octave) {
		src = __audioSynth.generate('piano', note, octave, 2);
	...
	var fnPlayKeyboard = function(e) {
		...
		if(keyboard[e.keyCode]) {
			...
			fnPlayChord(note, __octave + octaveModifier);
		} else {
			return false;	
		}
    ...
    var fnPlayChord = function(note, octave) {
        const thirds = { C : 'E', D : 'F', E : 'G', F : 'A', G : 'B', A : 'C+', B : 'D+' };
        fnPlayNote(note, octave);
        const thirdNote = thirds[note];
        if (thirdNote[1] === '+') {
            octave++;
        }
        fnPlayNote(thirdNote[0], octave);

        const fifthNote = thirds[thirdNote[0]];
        if (fifthNote[1] === '+') {
            octave++;
        }
        fnPlayNote(fifthNote[0], octave);
    }
